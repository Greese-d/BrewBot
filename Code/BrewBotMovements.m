    ...
    methods
        ...
        function moveRobot(obj, hObject, nova2Waypoints, nova2MovedObject, nova2Vertices, ur3eWaypoints, ur3eMovedObject, ur3eVertices)
            ...
            for i = 1:steps
                ...
                % Can be commented to stop using collision detection for baby
                obj.moveBaby(-0.01);

                % Check for emergency stop
                if checkEmergencyStop(obj, hObject)
                    error("EmergencyStop:Triggered", "Emergency stop activated, exiting function hierarchy.");
                end

                if obj.checkCollision()
                    error("Collision:Triggered", "Collision detected, exiting function hierarchy.");
                end
        ...
        function moveBaby(obj, displacement)
            % MOVE BABY - Translates the "baby" object by a specified displacement vector.
            % Arguments:
            %   displacement - A 3-element vector [dx, dy, dz] representing the translation in x, y, and z.
        
            % Get the current vertices of the baby object
            babyVertices = get(obj.baby, 'Vertices');
        
            % Apply translation transformation
            translationMatrix = transl(displacement(1), 0, 0);
            transformedVertices = [babyVertices, ones(size(babyVertices, 1), 1)] * translationMatrix';
        
            % Update the object's vertices
            set(obj.baby, 'Vertices', transformedVertices(:, 1:3));
        end

        %% Check for collision

        function isCollision = checkCollision(obj)
            % Check if robots or baby object are within any bounding boxes
            nova2Pos = obj.nova2.model.fkine(obj.nova2.model.getpos).t;
            ur3ePos = obj.ur3e.model.fkine(obj.ur3e.model.getpos).t;
            babyVertices = get(obj.baby, 'Vertices');
            babyPosition = mean(babyVertices, 1);  % Mean of baby vertices as position
            isCollision = false;
        
            for j = 1:size(obj.boundingBoxes, 1)
                boxMin = obj.boundingBoxes{j, 1};
                boxMax = obj.boundingBoxes{j, 2};
        
                % Check if Nova2, UR3e, or Baby is inside the bounding box
                if (all(nova2Pos >= boxMin') && all(nova2Pos <= boxMax')) || ...
                   (all(ur3ePos >= boxMin') && all(ur3ePos <= boxMax')) || ...
                   (all(babyPosition >= boxMin) && all(babyPosition <= boxMax))
                    isCollision = true;
                    disp('Collision detected! Movement stopped. Please reset');
                    return;
                end
            end
        end
